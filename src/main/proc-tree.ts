...
// Unambiguous AI CLI executables. A renamed/new CLI is a documented
// false-negative (follow-up: user-extensible setting); ambiguous binaries
// (goose=DB migrations, codex=LaTeX, gemini) are intentionally NOT matched
// bare to avoid flipping ordinary code dictation.
const DIRECT_AI_BINS = new Set(['claude', 'claude-code', 'cursor-agent', 'aider'])
// Wrappers whose argv may reference an AI CLI (npx/bunx/node ./.bin/claude).
const WRAPPERS = new Set(['node', 'bun', 'bunx', 'npm', 'npx', 'pnpm', 'yarn', 'deno'])
